...
2026-09-15 用户明确将图标右键改为与顶部菜单栏同样的应用菜单，移除独立固定模式开关。EdgeHandle 每次打开右键菜单调用同一 menuNeedsUpdate 构建逻辑，并设置相同菜单代理，使便笺列表、归档恢复、显示状态和操作保持一致；不复制菜单实现。Cmd 点击仍控制固定展示／隐藏。取代检查：本记录更新右键入口的当前事实，之前的单项固定模式右键入口被此交互取代，固定模式本身和其他交互仍有效，不归档整份记录。验证覆盖两处菜单内容一致、旧选项移除、新建动作、动态便笺列表，以及 Cmd 点击和拖动；2026-09-15 定向检查全部通过，文档检查通过。本次未重跑此前已知在当前环境失败的完整快捷键回归。
